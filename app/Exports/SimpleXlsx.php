...
class SimpleXlsx
{
    ...
    public function __construct(array $rows, array $headers, string $tempPrefix = 'export', string $title = 'Data')
    {
        $this->rows = $rows;
    ...
    public function toXlsx(): string
    {
        ...
        foreach ($this->rows as $row) {
            ...
            $sheetRows[] = array_pad($values, count($this->headers), '');
        }
...
XML);
        $coreXml = str_replace('{{title}}', htmlspecialchars($this->title, ENT_XML1, 'UTF-8'), <<<'XML'
<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
