...
<body>
    ...
    <div class="head"><div><h1>NOTA PENGADAAN BARANG</h1><p><b>{{ $procurementNote->number }}</b></p><p>Tanggal Nota: {{ $procurementNote->issued_at?->translatedFormat('d F Y, H:i') }}</p><p>Tanggal Cetak: {{ $procurementNote->last_printed_at?->translatedFormat('d F Y, H:i') }}</p></div><div><b>THI2-WAREHOUSE</b><p>Status: {{ $procurementNote->status }}</p><p>Driver: {{ $procurementNote->driver_name ?: '-' }}</p></div></div>
