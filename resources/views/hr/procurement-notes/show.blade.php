    ...
    <div class="space-y-6">
        ...
        @if($procurementNote->isDraft())
            <form method="POST" action="{{ route('hr.procurement-notes.update', $procurementNote) }}" data-submit-once class="overflow-hidden rounded-xl border border-amber-200 bg-white shadow-sm">
                @csrf @method('PUT')
                <div class="border-b border-amber-200 bg-amber-50 px-5 py-4"><h2 class="text-sm font-bold text-amber-900">Edit Draft</h2><p class="mt-1 text-xs text-amber-700">Cuplikan item akan dikunci setelah nota diterbitkan.</p></div>
